...
const ClosedTasksAdmin = () => {
  ...
  useEffect(() => {
    const fetchTasks = async () => {
      try {
        ...
        const filteredTasks = tasksSnapshot.docs
          ...
          .filter((task: any) => {
            const members = task.assigned?.members;
            if (!Array.isArray(members)) return false;

            // ✅ Check if members array has a label equal to flatNumber
            return members.some((member: any) => member.label === flatLabel);
          });
    ...
    fetchTasks();
  }, [customTasksCollectionName, flatLabel, societyName]);
    ...
    return (
      <View style={styles.cardview}>
        <TouchableOpacity
          onPress={() =>
            router.push({
              pathname: "/member/TasksMember/TaskDetailsMember",
              params: {
...
export default ClosedTasksAdmin;
